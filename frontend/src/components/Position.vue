<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            Position # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            Position
        </v-card-title >
...
<script>
    import Position from '../components/PositionBase.vue'
    ...
    export default {
        ...
        created(){
            ...
            websocketUrl.pathname = "/rsocket/positions";
            websocketUrl.hash = "";
            ...
            client.connect().subscribe({
                ...
                const endpoint = "positions."+ me.value.id +".get"
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/positions'), this.value)
                        } else {
